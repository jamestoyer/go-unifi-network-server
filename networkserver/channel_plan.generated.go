...
type responseBodyChannelPlan struct {
	Metadata json.RawMessage `json:"meta"`
	Payload  []ChannelPlan   `json:"data"`
}

func (c *Client) CreateChannelPlan(ctx context.Context, site string, data *ChannelPlan) (*ChannelPlan, *http.Response, error) {
	endpointPath := path.Join("api/s/", site, "rest", "channelplan")
	req, err := c.NewRequest(ctx, http.MethodPost, endpointPath, data)
	if err != nil {
		return nil, nil, err
	}

	var body responseBodyChannelPlan
	resp, err := c.Do(ctx, req, &body)
	if err != nil {
		return nil, resp, fmt.Errorf(`unable to create channelplan: %w`, err)
	}

	var item ChannelPlan
	switch len(body.Payload) {
	case 0:
		err = errors.New(`failed to create ChannelPlan`)
	case 1:
		item = body.Payload[0]
	default:
...
func (c *Client) GetChannelPlan(ctx context.Context, site, id string) (*ChannelPlan, *http.Response, error) {
	...
	resp, err := c.Do(ctx, req, &body)
	if err != nil {
		return nil, resp, fmt.Errorf(`unable to get ChannelPlan: %w`, err)
	}
...
func (c *Client) ListChannelPlan(ctx context.Context, site string) ([]ChannelPlan, *http.Response, error) {
	endpointPath := path.Join("api/s/", site, "rest", "channelplan")
	req, err := c.NewRequest(ctx, http.MethodGet, endpointPath, nil)
	if err != nil {
		return nil, nil, err
...
func (c *Client) UpdateChannelPlan(ctx context.Context, site string, data *ChannelPlan) (*ChannelPlan, *http.Response, error) {
	...
	resp, err := c.Do(ctx, req, &body)
	if err != nil {
		return nil, resp, fmt.Errorf(`unable to update channelplan: %w`, err)
	}

	var item ChannelPlan
	switch len(body.Payload) {
	case 0:
		err = errors.New(`failed to update ChannelPlan`)
	case 1:
		item = body.Payload[0]
	default:
		err = fmt.Errorf("unexpected number of results: %v", len(body.Payload))
